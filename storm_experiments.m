...
nStormPts = 360;
stormT  = [0 1 0 0 0 0 0 0]; % transition probabilities (on circle)
% Plane Parameters
fuel_rate = 5000/60;
planeX = 100;
planeY = 50;
...
% Save file
if saveFile
    saveName = sprintf('storm-%d-%d-plane-%d-%d-%d',stormX,stormY,planeX,planeY,timeStep);
    matName = sprintf('%s.mat', saveName);
    save(fullfile('storm', matName), 'g');
    fprintf('File saved as %s\n', saveName);
    videoName = sprintf('%s.avi', saveName);
