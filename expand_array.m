function exp_array=expand_array(node_x,node_y,world,hn,xTarget,yTarget,CLOSED,MAX_X,MAX_Y)
            ...
            if (k~=j || k~=0)  %The node itself is not its successor
                ...
                if( (s_x >0 && s_x <=MAX_X) && (s_y >0 && s_y <=MAX_Y))%node within array bound
                    ...
                    if (flag == 1)
                        ...
                        storm_x = world.storm.state(1);
                        storm_y = world.storm.state(2);
                        sigma = world.storm.sigma;
                        
                        exp_array(exp_count,3) = hn-world.cost(node_x,node_y...
                            ,storm_x,storm_y,sigma,s_x,s_y,s_x,s_y);%cost of travelling to node
